...
router.post('/', async (req, res) => {
    try {
        ...
        const { ID, nickname, password, confirm } = /*await userSchema.validateAsync(*/
            req.body
        /*)*/;
        ...
        // if (re_nickname.search(nickname) === -1) {
        //     return res.status(412).send({
        ...
        if (checkNickname(ID) === false) {
            res.status(412).send({errorMessage : "ID의 형식이 일치하지 않습니다."});
            return;
            };

        if (isRegexValidation(password, ID)) {
            return res.status(412).send({
